...
% 計算隨機生成常態分布的 標準差數量
diff_example = zeros(1,sample_number) ;
for i = 1:sample_number
    diff_example(i) = ( example(i) - mean(example) ) / std(example) ;
end

% 計算原資料的 標準差數量
diff_original = zeros(1,sample_number) ;
for i = 1:sample_number
    diff_original(i) = ( original_sample(i) - mean(original_sample) ) / std(original_sample) ;
end


after_process1 = zeros(1,sample_number) ;
for i = 1:sample_number
    after_process1(i) = original_sample(i) - o_ *  ( ( std(original_sample) / o_ ) *diff_original(i) - diff_example(i) ) ;
end


draw_temp = after_process1 ;

after_process1 = int8(after_process1) ;
...
yyaxis left
bar(sample_counter1) ;
title(['調整後樣本分數分布長條圖  (方法二)    \sigma^{2} = ' num2str((o_).^2)],['考生數量: ' num2str(sample_number)]);
xlabel('調整後樣本分數');	
ylabel('數量');	
